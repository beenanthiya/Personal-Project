...
module.exports = (app, db) => {
    ...
    // app.get('/user/:id', passport.authenticate('jwt', {
    //         session: false
    //     }),
    ...
    //             attributes: [
    //                 ['request_to_id', 'id'], 'status'
    //             ],
    //             where: {
    //                 request_from_id: userId,
    ...
    //                 res.status(200).send({
    //                     ...targetUser,
    //                     statusName: 'ขอเป็นเพื่อน'
    //                 })
    //             }
    //         }
    //     });

    app.put('/change-password', passport.authenticate('jwt', {
            session: false
        }),
        async function (req, res) {
            let targetUser = await db.user.findOne({
